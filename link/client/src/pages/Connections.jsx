...
const Connections = () => {
  ...
  const dataArray = [
    ...
    { label: 'FriendRequests', value: pendingConnections, icon: UserRoundPen },
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab) => (
              <button
                ...
                key={tab.label}
                className={`relative cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
                <tab.icon className='w-4 h-4' />
                <span className='ml-1'>{tab.label}</span>

                {/* 🔵 Blue Dot for Friend Requests */}
                {tab.label === 'FriendRequests' && tab.value.length > 0 && (
