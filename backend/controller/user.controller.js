...
const userSchema = z.object({
    ...
    username: z.string().min(3, { message: "Username should aleast have 3 character" }),
    password: z.string().min(6, { message: "Password should atleast be of length 6" }),
})
export const register = async (req, res) => {
    try {
        ...
        if (!email || !username || !password) {
            return res.status(400).json({ errors: "All field required" });
        }
